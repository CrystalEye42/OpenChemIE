...
class V1SCScoreTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.v1_scscore_url = "https://askcos-demo.mit.edu/api/v2/scscore/"
        cls.v1_celery_url = "https://askcos-demo.mit.edu/api/v2/celery/task"
        cls.legacy_adapter_url = f"http://{V2_HOST}:{V2_PORT}/api/legacy/scscore/"
    ...
    def test_1(self):
        case_file = "tests/adapters/v1_scscore/v1_scscore_test_case_1.json"
        with open(case_file, "r") as f:
            data = json.load(f)

        results = {}
        for mode, url in [("v1", self.v1_scscore_url),
        ...
        #for r1, r2 in zip(results["v1"], results["legacy"]):
        self.assertAlmostEqual(results["v1"]["score"], results["legacy"]["score"], places=4)
