...
class ForwardControllerTest(unittest.TestCase):
    """Test class for Forward Controller wrapper"""
    ...
    def setUpClass(cls) -> None:
        ...
        cls.module_url = f"{V2_HOST}:{V2_PORT}/api/forward"
    ...
    def test_1(self):
        case_file = "tests/wrappers/forward/forward_controller_test_case_1.json"
        ...
        for response in [response_sync, response_async]:
            ...
            self.assertIsInstance(response["result"][0][0], dict)
            self.assertIsInstance(response["result"][0][0]["rank"], int)
            self.assertIsInstance(response["result"][0][0]["outcome"], str)
            self.assertIsInstance(response["result"][0][0]["score"], float)
            self.assertIsInstance(response["result"][0][0]["prob"], float)
            self.assertIsInstance(response["result"][0][0]["mol_wt"], float)
