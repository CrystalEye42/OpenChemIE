        ...
        for j, reac_tag in enumerate(reac_atom_tags):
            ...
            if reac_tag not in changed_atom_tags:  # don't bother comparing if we know this atom changes
                ...
                if atoms_are_different(prod_atoms[i], reac_atoms[j]):
                    ...
                    break
                # If reac_tag appears multiple times, add (need for stoichiometry > 1)
...
def get_special_groups(mol):
    ...
    group_templates += {
        ...
        ((1, 2,), '[*]/[CH]=[CH0]([*])\[*]'),  # trans with one hydrogen
...
def convert_atom_to_wildcard(atom, super_general: bool = False):
...
def get_strict_smarts_for_atom(atom, super_general: bool = False):
    """
            ...
            if expansion:
                for atom in mol.GetAtoms():
                    ...
                    if label in expansion and label not in changed_atom_tags:
                        ...
                        symbol_replacements.append((
                            atom.GetIdx(),
                            convert_atom_to_wildcard(atom, super_general=super_general))
                        )
...
def extract_from_reaction_general(reaction, super_general: bool = False):
